...
define(['i18n!orion/navigate/nls/messages', 'orion/Deferred', 'orion/extensionCommands'], function(messages, Deferred, mExtensionCommands){
	...
	ProjectClient.prototype = /**@lends orion.ProjectClient.ProjectClient.prototype */ {
		...
		readProject : function(fileMetadata){
			var projectJsonLocation = fileMetadata.Location + "project.json"; //$NON-NLS-0$
			return this.fileClient.loadWorkspace().then(function(workspaceMetadata){
				return this.fileClient.read(projectJsonLocation, true).then(function(projectJsonData){
					if (projectJsonData.Directory) {
						return new Deferred().reject();
					}
					return this.fileClient.read(projectJsonLocation).then(function(content){
						try{
							var projectJson = content && content.length>0 ? JSON.parse(content) : {};
							projectJson.Name = projectJson.Name || fileMetadata.Name;
							projectJson.ContentLocation = fileMetadata.Location;
							projectJson.WorkspaceLocation = workspaceMetadata.Location;
							projectJson.ProjectJsonLocation = projectJsonData.Location;
							return projectJson;
						} catch (e){
