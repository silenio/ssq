	...
	objects.mixin(ProjectNavModel.prototype, /** @lends orion.sidebar.ProjectNavModel.prototype */ {
		...
		getId: function(item){
			if(item.type==="Project") { //$NON-NLS-0$
				...
				item = /*item.FileMetadata || */item.Dependency;
			}
	...
	objects.mixin(ProjectNavExplorer.prototype, /** @lends orion.sidebar.ProjectNavExplorer.prototype */ {
		...
		display: function(fileMetadata, redisplay){
			...
			return this.projectClient.readProject(fileMetadata).then(function(projectData){
				...
				Deferred.all((projectData.Dependencies || []).map(function(dependency) {
					var item = {Dependency: dependency, Project: projectData};
					projectData.children.push(item);
					return this.projectClient.getDependencyFileMetadata(dependency, projectData.WorkspaceLocation).then(function(dependencyMetadata) {
						objects.mixin(item, {
...
//						return this.expandItem(fileMetadata);
						return Deferred.all((projectData.Dependencies || []).concat(fileMetadata).map(function(item) {
